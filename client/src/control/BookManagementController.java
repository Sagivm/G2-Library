...
/**
 * BookManagementController is the controller that manage all books, domains,
 * subject and authors.
 * 
 * @author idanN
 */
public class BookManagementController {
	 ...
	 */
	@FXML
	public TableView<PropertyBook> BooksTableView;

	/**
	 * TableColumn of book sn
	 */
	@FXML
	public TableColumn BookSn;

	/**
	 * TableColumn of book title
	 */
	@FXML
	public TableColumn BookTitle;

	/**
	 * TableColumn of book authors
	 */
	@FXML
	public TableColumn BookAuthors;

	/**
	 * TableColumn of book keywords
	 */
	@FXML
	public TableColumn BookKeywords;

	/**
	 * TableColumn of book state of hide
	 */
	@FXML
	public TableColumn BookHide;
	...
	/**
	 * Button to move authors from all avalibale authors list to book authors
	 * list
	 */
	@FXML
	...
	/**
	 * Button to move authors from book authors list to all avalibale authors
	 * list
	 */
	@FXML
	...
	/**
	 * Button to move authors from all avalibale subjects list to book subjects
	 * list
	 */
	@FXML
	...
	/**
	 * Button to move authors from book subjects list to all avalibale subjects
	 * list
	 */
	@FXML
	...
	/**
	 * this attribute insert the (filtered books by search) to the table.
	 */
	...
	public void initialize() {
		...
		BookSummary.setStyle(
				"-fx-focus-color: -fx-control-inner-background ; -fx-faint-focus-color: -fx-control-inner-background ; -fx-background-insets: 0;-fx-background-color: transparent, white, transparent, white;");
				...
				synchronized (recv_getAuthors) {
					...
					Platform.runLater(() -> {
						...
						for (int i = 0; i < BooksList.size(); i += 9) {
							if (i + 9 < BooksList.size() && BooksList.get(i).equals(BooksList.get(i + 9))) {
								...
								if (authors.equals("")) {
									String hide;
									if (BooksList.get(i + 3).equals("0"))
										hide = "no";
									else
										hide = "yes";
									data.add(new PropertyBook(BooksList.get(i), BooksList.get(i + 1),
											BooksList.get(i + 2), hide, BooksList.get(i + 4), BooksList.get(i + 5),
											BooksList.get(i + 6), BooksList.get(i + 7), BooksList.get(i + 8)));
									filteredData.add(new PropertyBook(BooksList.get(i), BooksList.get(i + 1),
											BooksList.get(i + 2), hide, BooksList.get(i + 4), BooksList.get(i + 5),
											BooksList.get(i + 6), BooksList.get(i + 7), BooksList.get(i + 8)));
								} else {
									String hide;
									if (BooksList.get(i + 3).equals("0"))
										...
										hide = "yes";
									data.add(new PropertyBook(BooksList.get(i), BooksList.get(i + 1),
											BooksList.get(i + 2), hide, BooksList.get(i + 4),
											authors + BooksList.get(i + 5), BooksList.get(i + 6), BooksList.get(i + 7),
											BooksList.get(i + 8)));
									filteredData.add(new PropertyBook(BooksList.get(i), BooksList.get(i + 1),
											BooksList.get(i + 2), hide, BooksList.get(i + 4),
											authors + BooksList.get(i + 5), BooksList.get(i + 6), BooksList.get(i + 7),
					...
					Platform.runLater(() -> {
						...
						for (int i = 0; i < authorList.size(); i++) {
							names.add(i, "(" + authorList.get(i).getId() + ")" + "\t" + authorList.get(i).getFirstname()
		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				synchronized (recv_getSubjects) {
					...
					BookSn.setCellValueFactory(new PropertyValueFactory<pendingUser, String>("bookSn"));

					BookTitle.setCellValueFactory(new PropertyValueFactory<pendingUser, String>("bookTitle"));

					BookKeywords.setCellValueFactory(new PropertyValueFactory<pendingUser, String>("bookKeywords"));

					BookHide.setCellValueFactory(new PropertyValueFactory<pendingUser, String>("bookHide"));

					BookAuthors.setCellValueFactory(new PropertyValueFactory<pendingUser, String>("authorName"));
				}
						...
						.addListener((obs, oldSelection, newSelection) -> {
							if (newSelection != null) {
								...
								if (newSelection.getBookImage() != null) {
									...
									byte[] imageInBytes = javax.xml.bind.DatatypeConverter
											.parseBase64Binary(base64EncodedImage);
				...
				public void run() {
					...
					synchronized (recv_getBookTableOfContent) {
						...
						for (int i = 0; i < selectedAuthorsString.size(); i++) {
							namesselected.add(i,
									"(" + selectedAuthorsString.get(i).getId() + ")" + "\t"
											+ selectedAuthorsString.get(i).getFirstname() + " "
											+ selectedAuthorsString.get(i).getLastname());
						...
						ObservableList<String> languages = FXCollections.observableArrayList("English", "Hebrew",
								"Russian");
...
//		});
		...
		addBookBtn.setOnAction(e -> {
			...
			editBookPane.setVisible(false);
			ObservableList<String> languages = FXCollections.observableArrayList("English", "Hebrew", "Russian",
					"Arabic");
				...
				public void run() {
					...
					synchronized (recv_getAuthors) {
						...
						Platform.runLater(() -> {
							...
							// System.out.println(statush.get(1).getFirstname());
							for (int i = 0; i < authorList.size(); i++) {
								names.add(i, "(" + authorList.get(i).getId() + ")" + "\t"
										+ authorList.get(i).getFirstname() + " " + authorList.get(i).getLastname());
							}
							// System.out.println(names.get(0));
							ObservableList<String> authors = FXCollections.observableArrayList(names);
							addBookAuthorsList.setItems(authors);
		...
		submitAddBook.setOnAction(e -> {
			...
			for (int i = 0; i < Subjects.size(); i++)
				SubjectsList += Subjects.get(i).substring(Subjects.get(i).indexOf("(") + 1,
						Subjects.get(i).indexOf(")")) + "^";
			...
			else if (language == null || authorsId.length() == 0 || SubjectsList.length() == 0 || TitleBook == null
					|| keywords == null || tableOfContent == null || summary == null)
				actionOnError(ActionType.CONTINUE, "You must to fill the all fields!");

			else {
				Message message6 = prepareAddBook(ActionType.ADD_BOOK, Validate.fixText(TitleBook), authorsId, keywords,
						language, SubjectsList, tableOfContent, summary, picture, price);
				try {
					ClientController.clientConnectionController.sendToServer(message6);
				} catch (Exception e1) {
					e1.printStackTrace();
		...
		submitEditBook.setOnAction(e -> {
			...
			else if (language == null || authorsId.length() == 0 || SubjectsList.length() == 0
					|| TitleBook.equals("") == true || keywords.equals("") == true || tableOfContent.equals("") == true
					|| summary.equals("") == true)
				actionOnError(ActionType.CONTINUE, "You must to fill the all fields!");

			else {
				Message message6 = prepareEditBook(ActionType.EDIT_BOOK, Sn, TitleBook, authorsId, keywords, language,
						SubjectsList, tableOfContent, summary, picture, price);
				try {
					ClientController.clientConnectionController.sendToServer(message6);
				} catch (Exception e1) {
					e1.printStackTrace();
		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				synchronized (recv_getDomainsWithId) {
					...
					domainId.setCellValueFactory(new PropertyValueFactory<PropertyDomain, String>("domainId"));

					domainName.setCellValueFactory(new PropertyValueFactory<PropertyDomain, String>("domainName"));

					domainId.setStyle("-fx-alignment: CENTER;");
					domainName.setStyle("-fx-alignment: CENTER;");
					mainDomainTableView.setItems(dataDomains);

					mainDomainTableView.getSelectionModel().selectedItemProperty()
							.addListener((obs, oldSelection, newSelection) -> {
								if (newSelection != null) {
						...
						if (DomainName.equals(""))
							...
							try {
								...
								Parent root = FXMLLoader
										.load(getClass().getResource(ScreensInfo.HOMEPAGE_BOOK_MANAGEMENT));
								globalPane.getChildren().add(root);
							} catch (Exception e1) {
								e1.printStackTrace();
							}
								...
								Platform.runLater(new Runnable() {
									...
									public void run() {
										...
										synchronized (recv_getDomainsWithId) {
											...
											Platform.runLater(() -> {
												for (int i = 0; i < domainsList.size(); i += 2) {
													dataDomains.add(new PropertyDomain(domainsList.get(i),
															domainsList.get(i + 1)));
												}
											});
											...
											addDomainEditBtn.setOnAction(e -> {
												...
												addDomainPane.setVisible(false);
												editDomainPane.setVisible(true);
												PropertyDomain selectedItem = mainDomainTableView.getSelectionModel()
														.getSelectedItem();
												editDomainName.setText(selectedItem.getDomainName());
											});
											...
											editDomainSubmit.setOnAction(e -> {
												...
												if (DomainName.equals(""))
													actionOnError(ActionType.CONTINUE,
															"You must to fill the all fields!");
												else {
													Message message5 = prepareEditDomain(ActionType.EDIT_DOMAIN,
															DomainId, DomainName);
													try {
														ClientController.clientConnectionController
																.sendToServer(message5);
													} catch (Exception e1) {
														e1.printStackTrace();
													}
													...
													actionOnError(ActionType.CONTINUE,
															"The domain edited successfully!");
													try {
														while (globalPane.getChildren().size() > 0)
															globalPane.getChildren().remove(0);
														Parent root = FXMLLoader.load(getClass()
																.getResource(ScreensInfo.HOMEPAGE_BOOK_MANAGEMENT));
														globalPane.getChildren().add(root);
													} catch (Exception e1) {
														e1.printStackTrace();
													}
				...
				synchronized (recv_getSubjectsInfo) {
					...
					Platform.runLater(() -> {
						for (int i = 0; i < subjectsList.size(); i += 4) {
							dataSubjects.add(new PropertySubject(subjectsList.get(i), subjectsList.get(i + 1),
									"(" + subjectsList.get(i + 2) + ")" + " " + subjectsList.get(i + 3)));
						}
					});

					subjectId.setCellValueFactory(new PropertyValueFactory<PropertyDomain, String>("subjectId"));

					subjectName.setCellValueFactory(new PropertyValueFactory<PropertyDomain, String>("subjectName"));

					subjectDomain
							.setCellValueFactory(new PropertyValueFactory<PropertyDomain, String>("subjectDomain"));
					...
					mainSubjectTable.getSelectionModel().selectedItemProperty()
							.addListener((obs, oldSelection, newSelection) -> {
								if (newSelection != null) {
		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				synchronized (recv_getAuthors) {
					...
					Platform.runLater(() -> {
						for (int i = 0; i < authorList.size(); i++) {
							dataAuthors.add(new PropertyAuthor(authorList.get(i).getId(),
									authorList.get(i).getFirstname(), authorList.get(i).getLastname()));
						}
					});
					authorId.setCellValueFactory(new PropertyValueFactory<PropertyDomain, String>("authorId"));

					authorFirstName
							.setCellValueFactory(new PropertyValueFactory<PropertyDomain, String>("authorFirstName"));

					authorLastName
							.setCellValueFactory(new PropertyValueFactory<PropertyDomain, String>("authorLastName"));
					...
					mainAuthorTableView.getSelectionModel().selectedItemProperty()
							.addListener((obs, oldSelection, newSelection) -> {
								if (newSelection != null) {
									...
									for (int i = 0; i < BooksList.size(); i += 9) {
										if (i + 9 < BooksList.size() && BooksList.get(i).equals(BooksList.get(i + 9))) {
											...
											if (authors.equals("")) {
												...
												data.add(new PropertyBook(BooksList.get(i), BooksList.get(i + 1),
														BooksList.get(i + 2), hide, BooksList.get(i + 4),
														BooksList.get(i + 5), BooksList.get(i + 6),
														BooksList.get(i + 7), BooksList.get(i + 8)));
												filteredData
														.add(new PropertyBook(BooksList.get(i), BooksList.get(i + 1),
																BooksList.get(i + 2), hide, BooksList.get(i + 4),
																BooksList.get(i + 5), BooksList.get(i + 6),
																BooksList.get(i + 7), BooksList.get(i + 8)));
											} else {
												...
												data.add(new PropertyBook(BooksList.get(i), BooksList.get(i + 1),
														BooksList.get(i + 2), hide, BooksList.get(i + 4),
														authors + BooksList.get(i + 5), BooksList.get(i + 6),
														BooksList.get(i + 7), BooksList.get(i + 8)));
												filteredData
														.add(new PropertyBook(BooksList.get(i), BooksList.get(i + 1),
																BooksList.get(i + 2), hide, BooksList.get(i + 4),
...
public void PressedDelete(ActionEvent e)
{
	...
	/**
	 * Update books table view after change the search input (filteredTextField
	 * string)
	 */
	...
	/**
	 * this function help to updateFilteredData function by searching in the
	 * data (books table) the appropriate string that user search
	 * 
	 * @param book
	 *            The book.
	 ...
	 * 
	 * @param type
	 *            the type of the action that server need to do
	 * @return message
	 */
	 ...
	 * @param subjectsList2
	 *            ID's of the subjects
	 * @param tableOfContent
	 *            table of content of the book
	 * @param summary
	 *            summary of the book
	 * @param picture
	 *            picture of the book (base64 encoded)
	 * @param price
	 *            price of the book
	 * @return message
	 */
	public Message prepareAddBook(ActionType type, String titleBook, String authorsId2, String keywords,
			String language, String subjectsList2, String tableOfContent, String summary, String picture,
			String price) {
		Message message = new Message();
		ArrayList<String> elementsList = new ArrayList<String>();
	 ...
	 *            new summary
	 * @param picture
	 *            new picture (base64 encoded)
	 * @param price
	 *            new price
	 * @return message
	 */
	public Message prepareEditBook(ActionType type, String Sn, String titleBook, String authorsId2, String keywords,
			String language, String subjectsList2, String tableOfContent, String summary, String picture,
			String price) {
		Message message = new Message();
		ArrayList<String> elementsList = new ArrayList<String>();
	...
	public static class PropertyBook {
		 ...
		 * @param bookKeywords
		 *            Gets the keywords of book.
		 * @param bookHide
		 *            Gets the hide state of book.
		 * @param authorId
		 *            Gets the author ID of book.
		 * @param authorName
		 *            Gets the author name of book.
		 * @param bookSummary
		 *            Gets the summary of book.
		 * @param bookImage
		 *            Gets the image of book.
		 * @param bookPrice
		 *            Gets the price of book.
		 */
		public PropertyBook(String bookSn, String bookTitle, String bookKeywords, String bookHide, String authorId,
				String authorName, String bookSummary, String bookImage, String bookPrice) {
	...
	public static class PropertySubject {
		 ...
		 * 
		 * @param subjectId
		 *            Gets the ID of subject.
		 * @param subjectName
		 *            Gets the name of subject.
		 * @param subjectDomain
		 *            Gets the domain of subject.
		 */
...
/**
 * This class makes sure the information from the server was received
 * successfully.
 * 
 * @author itain
 */
